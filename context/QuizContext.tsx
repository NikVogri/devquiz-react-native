...
interface QuizContextInterface {
	...
	findQuiz: (quizId: number) => Promise<void>;
...
export const QuizProvider = ({ children }: any) => {
	...
	const findQuiz = async (quizId: number) => {
		...
		if (foundQuiz) {
			if (foundQuiz.locked) {
				const purchasedQuizes = await getData(`purchased-quizes`);
				if (purchasedQuizes) {
					foundQuiz.locked = !purchasedQuizes.some(
						(purchasedQuizId: number) =>
							purchasedQuizId === foundQuiz.id
					);
				}
			}
			...
			if (foundQuiz.questionsAndAnswers) {
				...
				if (localQuizData) {
					const isFinished =
						localQuizData.lastCompletedStep ===
						foundQuiz.totalQuestions;
	...
	const handleAnswer = async (answerId: number) => {
		...
		if (answer?.isCorrect) {
			setCorrectAnswers(
				(prevCorrectCount: number) => prevCorrectCount + 1
			);
	...
	/**
	 * @description executes when clicking complete quiz button
	 */
