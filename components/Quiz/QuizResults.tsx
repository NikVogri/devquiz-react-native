...
import AwardsContext, { Award, AwardType } from "../../context/AwardsContext";
...
const QuizResults = ({ navigation }: { navigation: any }) => {
	...
	const { correctAnswers, quiz, restartQuiz, completeQuiz } = useContext(
		QuizContext
	);
	...
	const handleMarkAsCompleted = async () => {
		try {
			...
			if (
				!awards ||
				!awards.some((quizAward: Award) => quizAward.quizId === quiz.id)
			) {
				await pushLocalAward(quiz.id, AwardType.quiz);
				openModal(Modal.quizCompleted);
			}
