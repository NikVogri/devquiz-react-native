	...
	const getAllSavedQuizes = async () => {
		try {
			...
			for (const quiz of quizList) {
				...
				if (purchasedQuizes && quiz.locked) {
					quiz.locked = !purchasedQuizes.some(
						(purchasedQuizId: number) => purchasedQuizId === quiz.id
					);
				}
			}
			...
			for (const dataKey of dataKeys) {
				...
				quizList[quizId].completedQuestions =
